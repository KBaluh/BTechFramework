...
var BTechFramework = { Version : 'v1.7.1', ModifyDate : '20.02.2013' };
...
function GetMonthName(MonthIndex, Launge, Multipli) {
	...
	var MonthUA2 = ["Січня", "Лютого", "Березня", "Квітня", "Травня", "Червня", 
...
function GetParamValueFromBPItem(Item, ParameterName) {
	var Value = Item.ParentItems.ParentDiagram.Parameters.
		ItemsByName(ParameterName).Value;
	return Value;
}
...
function SetParamValueFromBPItem(Item, ParameterName, Value) {
	var Result = false;
	try {
		Item.ParentItems.ParentDiagram.Parameters.
			ItemsByName(ParameterName).Value = Value;
		Result = true;
	} catch (e) {
		BTCatchException('Ошибка изменения значения параметру: ' + 
